<div class="space-y-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
    <div class="kid-card h-fit lg:sticky lg:top-6">
        <h1 class="kid-title">Points Store</h1>
        <p class="mt-1 text-slate-600">Create and manage redeemable point store items.</p>

        <form wire:submit="{{ $editingId ? 'updateItem' : 'createItem' }}" class="mt-6 grid gap-4 md:grid-cols-2" autocomplete="off">
            <div>
                <label class="mb-1 block text-sm font-semibold text-slate-700">Title</label>
                <input wire:model.live="formTitle" type="text" class="w-full rounded-xl border border-slate-300 px-3 py-2" placeholder="Pick Dessert">
            ...
            <div>
                ...
                <input wire:model.live="formPoints" type="number" min="1" class="w-full rounded-xl border border-slate-300 px-3 py-2" placeholder="30">
            ...
            <div class="md:col-span-2">
                ...
                @if($formImage || $currentImagePath)
                    <div class="mt-3 flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                        <img src="{{ $formImage ? $formImage->temporaryUrl() : \Illuminate\Support\Facades\Storage::url($currentImagePath) }}" alt="Store item image preview" class="h-14 w-14 rounded-xl object-cover bg-white p-1">
            ...
            <div class="md:col-span-2 flex flex-wrap gap-3">
                <button type="submit" class="kid-btn kid-btn-primary">{{ $editingId ? 'Update Item' : 'Add Item' }}</button>
    ...
    <div class="kid-card !m-0">
        <h2 class="text-kid-xl font-bold text-slate-800">Store Items</h2>

        <div class="mt-4 grid gap-4 grid-cols-1 2xl:grid-cols-2">
            @forelse($items as $item)
                <div class="rounded-2xl border border-slate-200 p-4">
                    @if($item->image_path)
                        <img src="{{ \Illuminate\Support\Facades\Storage::url($item->image_path) }}" alt="{{ $item->title }} image" class="mb-3 h-24 w-24 rounded-xl object-cover bg-slate-100 p-1" />
                    @endif
                    <div class="flex items-start justify-between gap-3">
                        <h3 class="text-lg font-bold text-slate-800">{{ $item->title }}</h3>
                        <span class="rounded-full bg-amber-100 px-2 py-1 text-sm font-bold text-amber-700">{{ $item->points }} pts</span>
                    </div>
                    ...
                    <p class="mt-2 text-sm font-semibold {{ $item->active ? 'text-green-600' : 'text-slate-500' }}">{{ $item->active ? 'Active' : 'Inactive' }}</p>

                    <div class="mt-3 flex gap-2">
                        <button type="button" wire:click.prevent="editItem({{ $item->id }})" class="rounded-xl bg-blue-500 px-3 py-2 text-sm font-bold text-white">Edit</button>
                        <button type="button" wire:click.prevent="deleteItem({{ $item->id }})" wire:confirm="Delete {{ $item->title }}?" class="rounded-xl bg-red-500 px-3 py-2 text-sm font-bold text-white">Delete</button>
                    </div>
                </div>
            @empty
                <p class="text-slate-600">No items yet. Add one above.</p>
            @endforelse
        </div>
    </div>
</div>
